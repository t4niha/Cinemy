...
$query = 
    ...
    WHERE mr.series_id =? AND mr.user_profile_id =? LIMIT 1;";
$stmt = $conn->prepare($query);
$stmt->bind_param("ii", $series_id, $user_profile_id);
$stmt->execute();
$result=$stmt->get_result();
...
if (isset($_POST['delete_review_series_id'])) {
    $delete_review_series_id = $_POST['delete_review_series_id'];
    // DB query: delete series review
    $query = 
        "DELETE FROM series_reviews
        WHERE series_id = ? AND user_profile_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('ii', $delete_review_series_id, $user_profile_id);
    $stmt->execute();
    $stmt->close();
    header("Location: series_info.php?series_id=$delete_review_series_id");
    exit();
}

// Add new series review
if (isset($_POST['rating'])) {
    ...
    $new_series_id = $_POST['new_series_id'];
    $date_added = date('Y-m-d');
    $time_added = date('H:i:s');
    // DB query: Insert new series review
    $query = 
        "INSERT INTO series_reviews (review_text, rating, date_added, user_profile_id, series_id, time_added) VALUES (?,?,?,?,?,?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('sdsiis', $review_text, $rating, $date_added, $user_profile_id, $new_series_id, $time_added);
    $stmt->execute();
    $stmt->close();
    header("Location: series_info.php?series_id=$new_series_id");
    exit();
}

// DB query: if series already in watchlist
$query = 
    "SELECT COUNT(*) 
    FROM series_watchlists mw
    JOIN watchlists w ON mw.series_watchlist_id = w.list_id
    WHERE series_id =? AND user_profile_id =?";
$stmt = $conn->prepare($query);
$stmt->bind_param('ii', $series_id, $user_profile_id);
$stmt->execute();
...
if (isset($_POST['delete_series_id'])) {
    ...
    $query = 
        ...
        FROM series_watchlists mw
        JOIN watchlists w ON mw.series_watchlist_id = w.list_id
        WHERE mw.series_id = ? AND w.user_profile_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('ii', $series_id, $user_profile_id);
    $stmt->execute();
    $stmt->close();
    header("Location: series_info.php?series_id=$series_id");
    exit();
}

// Add to watchlist
if (isset($_POST['add_series_id'])) {
    $series_id = $_POST['add_series_id'];
    ...
    // DB query: Insert series into watchlist
    $date_adding = date('Y-m-d');
    $time_added = date('H:i:s');
    $query = 
        "INSERT INTO series_watchlists (series_watchlist_id, series_id, date_added, time_added)
        VALUES (?,?,?,?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('iiss', $list_id, $series_id, $date_adding, $time_added);
    $stmt->execute();
    $stmt->close();
    header("Location: series_info.php?series_id=$series_id");
    exit();
...
<header>
    ...
    <h1>▶ CINEMY</h1>
    <form action="series_info.php" method="POST" style="display: inline;">
            ...
            <?php if (strtolower($subscription_status)=='active'): ?>
                <?php if ($is_added>0): ?>
                    <form action="series_info.php" method="POST">
                        <input type="hidden" name="delete_series_id" value="<?php echo $content_info['id']; ?>">
                        <button type="submit" class="addremove_button">Remove From Watchlist</button>
                    </form>
                <?php else: ?>
                    <form action="series_info.php" method="POST">
                        <input type="hidden" name="add_series_id" value="<?php echo $content_info['id']; ?>" target="_blank">
    ...
    <?php if ($subscription_status == 'active'): ?>
        <?php if ($this_review): ?>
            <div class="content-box">
                ...
                <div class="button-container">
                    <!-- Delete review -->
                    <form action="series_info.php" method="POST">
                        <input type="hidden" name="delete_review_series_id" value="<?php echo $content_info['id']; ?>">
        ...
        <?php else: ?>
            <div class="content-box">
                ...
                <form action="series_info.php?series_id=<?php echo $series_id; ?>" method="POST">
                    ...
                    <input type="hidden" name="new_series_id" value="<?php echo $content_info['id']; ?>">
                    <div class="button-container">
