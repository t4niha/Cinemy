...
// DB query: retrieve reviewed movies and series
$query =
    "SELECT 
        ...
        mr.date_added, CONCAT(mr.date_added, ' ', mr.time_added) AS datetime_added,
        mr.review_text, mr.rating AS review_rating
    FROM movies m
    ...
    JOIN movie_reviews mr ON m.movie_id = mr.movie_id AND mr.user_profile_id = ?
    GROUP BY m.movie_id
    ...
    SELECT 
        ...
        sr.date_added, CONCAT(sr.date_added, ' ', sr.time_added) AS datetime_added,
        sr.review_text, sr.rating AS review_rating
    FROM series s
    ...
    JOIN series_reviews sr ON s.series_id = sr.series_id AND sr.user_profile_id = ?
...
// Filter
$filtered_list = array_filter($reviewed_list, function ($content) use ($filter_content_type, $filter_genre) {
...
<main>
    ...
    <?php if ($subscription_status == 'active'): ?>
        <div class="filter-box">
            <form action="reviews.php" method="POST" class="filter-form">
                <!-- Content type filter -->
                ...
                <select name="filter_order_by" id="filter_order_by" style="width: 100%; width: 220px; border: 1px solid #ccc; border-radius: 4px;">
                    ...
                    <option value="review_rating" <?php echo ($filter_order_by == 'review_rating') ? 'selected' : ''; ?>>Review Rating</option>
        ...
        <?php if ($filtered_list): ?>
            <?php foreach ($filtered_list as $content): ?>
                <div class="list-item-box">
                    ...
                    <span style="font-size: 20px;">(<?php $release_date = new DateTime($content['release_date']); echo $release_date->format('Y');?>)</span> - 
                    <span style="font-size: 20px;"><strong><?php echo htmlspecialchars($content['review_rating']); ?></strong></span><br>
                    <?php if ($content['review_text']): ?>
                        <span style="font-size: 15px;"><em>"<?php echo htmlspecialchars($content['review_text']); ?>"</em></span><br>
                    <?php endif; ?>
                    <span style="font-size: 14px;">~ <?php echo htmlspecialchars($content['date_added']) . '</span>'; ?>
