...
$query = "SELECT COUNT(rating) FROM movie_reviews WHERE movie_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $movie_id);
$stmt->execute();
...
$query = "SELECT COUNT(movie_watchlist_id) FROM movie_watchlists WHERE movie_id =?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $movie_id);
$stmt->execute();
...
// DB query: get this admin's review for this movie
$admin_pick_review = '';
$pick_type = 'movie';
$query = "SELECT admin_pick_review, date_added FROM admin_picks WHERE admin_account_id = ? AND admin_pick_type = ? AND movie_ID = ?;";
$stmt = $conn->prepare($query);
$stmt->bind_param("isi", $user_account_id, $pick_type, $movie_id);
$stmt->execute();
$result = $stmt->get_result();
$pick_details = [];
...
if (isset($_POST['delete_content'])) {
    ...
    header("Location: admin_movie.php?movie_id=$delete_id");
    exit();
}

// Add pick review
if (isset($_POST['post_pick'])) {
    $add_movie_id = $_POST['post_pick'];
    $admin_pick_review = $_POST['pick_review'];
    $admin_pick_review = str_replace(['"', "'"], '', $admin_pick_review);
    $date_added = date('Y-m-d');
    $admin_pick_type = 'movie';
    $admin_account_id = $user_account_id;
    // DB query: add movie pick
    $query = 
        "INSERT INTO admin_picks (admin_pick_type, movie_id, admin_account_id, admin_pick_review, date_added)
        VALUES (?,?,?,?,?);";
    $stmt = $conn->prepare($query);
    $stmt->bind_param('siiss', $admin_pick_type, $add_movie_id, $admin_account_id, $admin_pick_review, $date_added);
    $stmt->execute();
    $stmt->close();
    header("Location: admin_movie.php?movie_id=$add_movie_id");
    exit();
}

// Delete pick review
if (isset($_POST['delete_pick'])) {
    $delete_movie_id = $_POST['delete_pick'];
    ...
    $stmt->execute();
    $stmt->close();
    header("Location: admin_movie.php?movie_id=$delete_movie_id");
    exit();
}
...
if(isset($_POST['update_content'])) {
    ...
    $stmt = $conn->prepare("UPDATE movies 
                            SET movie_title = ?, movie_link = ?, movie_description = ?, imdb_rating = ?, rotten_tomatoes_rating = ?, release_date = ?, movie_director_id = ? 
                            WHERE movie_id = ?");
    $stmt->bind_param("sssdisii", $title, $link, $description, $imdb_rating, $rotten_rating, $release_date, $director_id, $movie_id);
    $stmt->execute();
    $stmt->close();
    ...
    <?php else: ?>
        ...
        <div class="content-box">
            ...
            <form action="admin_movie.php" method="POST">
                ...
                <!-- Update button -->
                <input type="hidden" name="update_content" value="<?= htmlspecialchars($movie_id) ?>">
