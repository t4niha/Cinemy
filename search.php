...
<main>
    <div class="filter-box">
        <form action="search.php" method="POST" class="filter-form">
            ...
            <select name="filter_content_type" id="filter_content_type">
    ...
    <div class="show-box">
            <div class="showing-box">
                <h3>Showing results for: <span style="font-weight: normal;">"<?php echo htmlspecialchars($search_query); ?>"</span></h3>
            </div>
        </div>
    <?php if ($filtered_list): ?>
        <?php foreach ($filtered_list as $content): ?>
            <div class="list-item-box">
                ...
                <?php if ($content['content_type'] == 'movie'): ?> <p>Movie: <?php echo strtoupper(htmlspecialchars($content['genres'])); ?></p>
                <?php elseif ($content['content_type'] == 'series'): ?> <p>Series: <?php echo strtoupper(htmlspecialchars($content['genres'])); ?></p>
                <?php endif; ?>
                <p><?php echo htmlspecialchars($content['description']); ?></p>
                <?php if ($content['content_type'] == 'movie'): ?>
                    <p><strong>Directed by </strong> <?php echo htmlspecialchars($content['director']); ?><br>
                <?php endif; ?>
                <strong>Starring </strong> <?php echo htmlspecialchars($content['actors']); ?></p>
                <?php if ($content['awards']): ?>
                    <p><strong>Awards & Nominations: </strong> <?php echo htmlspecialchars($content['awards']); ?><br>
                <?php endif; ?>
                <?php if ($content['imdb_rating']): ?>
                    <strong>IMDb: </strong><?php echo htmlspecialchars($content['imdb_rating']); ?><br>
                <?php endif; ?>
                <?php if ($content['rotten_tomatoes_rating']): ?>
                    <strong>Rotten Tomatoes: </strong> <?php echo htmlspecialchars($content['rotten_tomatoes_rating']); ?>%</p>
                <?php endif; ?>

                <!-- Go to Page buttons -->
                <?php if ($content['content_type'] == 'movie'): ?>
                    <form action="movie_info.php" method="POST">
                        <input type="hidden" name="movie_id" value="<?php echo $content['content_id']; ?>">
                ...
                <?php elseif ($content['content_type'] == 'series'): ?>
                    <form action="series_info.php" method="POST">
                        <input type="hidden" name="series_id" value="<?php echo $content['content_id']; ?>">
                        <button type="submit" class="go-to-link">Go to Page →</button>
                    </form>
                <?php endif; ?>
            </div>
